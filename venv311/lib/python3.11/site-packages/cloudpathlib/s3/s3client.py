...
class S3Client(Client):
    ...
    def _list_dir(
        self, cloud_path: S3Path, recursive=False
            ...
            for result_key in result.get("Contents", []):
                ...
                for parent in PurePosixPath(o_relative_path).parents:
                    ...
                    if parent_canonical not in yielded_dirs and str(parent) != ".":
                        yield (
                            self.CloudPath(
                                f"s3://{cloud_path.bucket}/{parent_canonical}"
                            ),
                            True,
                        )
                ...
                # yield object as file
                    ...
                    yield (
                        self.CloudPath(
                            f"s3://{cloud_path.bucket}/{result_key.get('Key')}"
                        ),
                        False,
                    )
    ...
    def _remove(self, cloud_path: S3Path, missing_ok: bool = True) -> None:
        ...
        if file_or_dir == "file":
            ...
            resp = bucket.objects.filter(
                Prefix=prefix, **self.boto3_list_extra_args
            ).delete(**self.boto3_list_extra_args)
    ...
    def _upload_file(
        ...
        obj.upload_file(
            str(local_path), Config=self.boto3_transfer_config, ExtraArgs=extra_args
        )
        return cloud_path
