...
class GSClient(Client):
    ...
    def _list_dir(
        self, cloud_path: GSPath, recursive=False
    ) -> Iterable[Tuple[GSPath, bool]]:
        ...
        if not cloud_path.bucket:
            ...
            yield from (
                (self.CloudPath(f"gs://{str(b)}"), True)
            ...
            for o in bucket.list_blobs(prefix=prefix):
                ...
                for parent in PurePosixPath(o.name[len(prefix) :]).parents:
                    ...
                    if parent not in yielded_dirs and str(parent) != ".":
                        yield (
                            self.CloudPath(
                                f"gs://{cloud_path.bucket}/{prefix}{parent}"
                            ),
                            True,  # is a directory
                        )
                        yielded_dirs.add(parent)
                yield (
                    self.CloudPath(f"gs://{cloud_path.bucket}/{o.name}"),
                    False,
                )  # is a file
        else:
    ...
    def _upload_file(
        self, local_path: Union[str, os.PathLike], cloud_path: GSPath
    ) -> GSPath:
        bucket = self.client.bucket(cloud_path.bucket)
