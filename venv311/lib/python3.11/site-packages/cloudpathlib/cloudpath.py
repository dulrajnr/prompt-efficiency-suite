...
if sys.version_info >= (3, 12):
    ...
    from pathlib import (
        _make_selector as _make_selector_pathlib,  # type: ignore[attr-defined]
    )
...
implementation_registry: Dict[str, CloudImplementation] = defaultdict(
    CloudImplementation
)
...
class CloudPath(metaclass=CloudPathMeta):
    ...
    def _walk_results_from_tree(root, tree, top_down=True):
        ...
        for dir in dirs:
            yield from CloudPath._walk_results_from_tree(
                root / dir, tree[dir], top_down=top_down
            )
    ...
    def walk(
        ...
        try:
            file_tree = self._build_subtree(
                recursive=True
            )  # walking is always recursive
    ...
    def open(
        ...
        if mode == "x" and self.exists():
            raise CloudPathFileExistsError(
                f"Cannot open existing file ({self}) for creation."
            )
    ...
    def __truediv__(self, other: Union[str, PurePosixPath]) -> Self:
        if not isinstance(other, (str, PurePosixPath)):
            raise TypeError(
                f"Can only join path {repr(self)} with strings or posix paths."
            )
    ...
    def stat(self, follow_symlinks: bool = True) -> os.stat_result:
        ...
        return self._dispatch_to_local_cache_path(
    ...
    def upload_from(
        ...
        if source.is_dir():
            for p in source.iterdir():
                (self / p.name).upload_from(
                    p, force_overwrite_to_cloud=force_overwrite_to_cloud
                )
            ...
            dst._upload_file_to_cloud(
                source, force_overwrite_to_cloud=force_overwrite_to_cloud
            )
...
class _CloudPathSelectable:
    ...
    def walk(self):
        ...
        with self.scandir(self) as items:
            ...
            yield self, [f.name for f in dirs_files[True]], [
                f.name for f in dirs_files[False]
            ]
