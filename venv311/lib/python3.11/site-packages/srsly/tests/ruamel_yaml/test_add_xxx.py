...
def test_dice_constructor():
    ...
    srsly.ruamel_yaml.add_constructor("!dice", dice_constructor)
...
def test_dice_constructor_with_loader():
    ...
    with pytest.raises(ValueError):
        srsly.ruamel_yaml.add_constructor(
            "!dice", dice_constructor, Loader=srsly.ruamel_yaml.Loader
        )
        data = srsly.ruamel_yaml.load(
            "initial hit points: !dice 8d4", Loader=srsly.ruamel_yaml.Loader
        )
...
def test_dice_implicit_resolver():
    ...
    with pytest.raises(ValueError):
        srsly.ruamel_yaml.add_implicit_resolver("!dice", pattern)
        assert (
            srsly.ruamel_yaml.dump(
                dict(treasure=Dice(10, 20)), default_flow_style=False
            )
            == "treasure: 10d20\n"
        )
