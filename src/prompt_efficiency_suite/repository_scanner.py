...
class RepositoryScanner:
    ...
    def scan_repository(self, repo_path: str) -> List[PromptLocation]:
        ...
        for file_path in files_to_scan:
            futures.append(
                self.executor.submit(self._scan_file, str(file_path))
            )
    ...
    def scan_repository(
        ...
        with ThreadPoolExecutor(max_workers=params.get('max_workers', 4)) as executor:
            futures = [
                executor.submit(self._scan_file, file_path, params)
                for file_path in files_to_scan
            ]
    ...
    def get_scan_stats(self) -> Dict[str, Any]:
        ...
        total_files = sum(
            result.file_stats['total_files']
            for result in self.scan_history
        )
        total_prompts = sum(
            len(result.found_prompts)
            for result in self.scan_history
        )
