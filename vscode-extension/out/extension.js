...
function activate(context) {
    ...
    const configPanelDisposable = vscode.commands.registerCommand('prompt-efficiency.openConfig', () => {
        ...
        configPanel = vscode.window.createWebviewPanel('promptEfficiencyConfig', 'Prompt Efficiency Settings', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    const analyzeDisposable = vscode.commands.registerCommand('prompt-efficiency.analyze', async () => {
        ...
        try {
            ...
            const panel = vscode.window.createWebviewPanel('promptAnalysis', 'Prompt Analysis Results', vscode.ViewColumn.Beside, {});
    ...
    const estimateCostDisposable = vscode.commands.registerCommand('prompt-efficiency.estimate-cost', async () => {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`Estimated cost: ${cost.total_cost} ${cost.currency}`);
        }
        catch (error) {
    ...
    const scanRepositoryDisposable = vscode.commands.registerCommand('prompt-efficiency.scan-repository', async () => {
        ...
        try {
            ...
            const panel = vscode.window.createWebviewPanel('repositoryScan', 'Repository Scan Results', vscode.ViewColumn.Beside, {});
    ...
    context.subscriptions.push(checkConnectionDisposable, configPanelDisposable, analyzeDisposable, optimizeDisposable, estimateCostDisposable, scanRepositoryDisposable, translateModelDisposable);
...
function setupConnectionCheck() {
    const config = vscode.workspace.getConfiguration('promptEfficiency');
    const autoCheck = config.get('autoCheckConnection');
    const interval = config.get('connectionCheckInterval');
...
function getConfigPanelHtml() {
    ...
    const defaultCurrency = config.get('defaultCurrency') || 'USD';
    const autoCheckConnection = config.get('autoCheckConnection') ?? true;
    const connectionCheckInterval = config.get('connectionCheckInterval') || 30000;
    return `
        <!DOCTYPE html>
        <html>
        <head>
...
function getScanResultsHtml(results) {
    return `
        <!DOCTYPE html>
        <html>
        <head>
