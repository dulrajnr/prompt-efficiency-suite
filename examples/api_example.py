...
def create_sample_files():
    ...
    domain_dict = {
        ...
        "compound_terms": ["API key", "rate limiting", "error message", "status code"],
        "preserve_patterns": [
            r"[A-Z]{2,}",  # Uppercase acronyms
            r"https?://\S+",  # URLs
        ],
        "remove_patterns": [r"TODO", r"FIXME", r"NOTE:"],
    }
...
def main():
    ...
    try:
        ...
        response = requests.post(
            f"{BASE_URL}/trim",
            headers=headers,
            json={"text": input_text, "domain": domain_path, "preserve_ratio": 0.8},
        )
        response.raise_for_status()
        ...
        response = requests.get(f"{BASE_URL}/budget/metrics", headers=headers)
        response.raise_for_status()
        ...
        response = requests.post(f"{BASE_URL}/cicd/tests", headers=headers)
        response.raise_for_status()
