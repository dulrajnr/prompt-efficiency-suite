...
from prompt_efficiency_suite.tester import PromptTester, TestCase, TestResult, TestSuite
...
def test_run_test_case(tester, sample_test_case):
    ...
    result = tester.run_test_case(test_case=sample_test_case, model=ModelType.GPT4)
...
def test_metadata_handling(tester):
    ...
    test_case = TestCase(name="Metadata Test", prompt="Test prompt", metadata=metadata)

    result = tester.run_test_case(test_case=test_case, model=ModelType.GPT4)
