import pytest
from prompt_efficiency_suite.model_translator import ModelTranslator, ModelType, ModelConfig, TranslationResult
import json
...
def test_translate_prompt_basic(translator):
    ...
    result = translator.translate(
        prompt=prompt,
        source_format=ModelType.OPENAI,
        target_format=ModelType.ANTHROPIC
    )
...
def test_invalid_model_type(translator):
    ...
    with pytest.raises(ValueError):
        translator.translate(
            prompt="Test",
            source_format="invalid_model",
            target_format=ModelType.OPENAI
        )
